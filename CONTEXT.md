...
**模型知识库 (Model Library)**:
结构化存储的数学模型条目集合（名称 / 分类 / 特点标签 / 适用说明），范围以历年国赛真题驱动的方法清单为准。供 Layer 2 建模前检索候选模型。
_Avoid_: 模型清单、方法清单（指本项目内的模型知识库时）

**候选模型池 (Candidate Model Pool)**:
按题目特点从模型知识库检索出的 Top-K 模型子集，注入建模师第一轮供其选择（可超出池子）。检索或提炼失败时降级为全量谱系。
_Avoid_: 候选列表、推荐模型（指候选模型池时）

**特点标签 (Model Trait)**:
描述数学模型适用前提的可枚举属性（样本量、线性/非线性、可解释性、机理/数据驱动、时间/截面等），是检索的语义匹配依据。
_Avoid_: 特征、标签（指特点标签时）
